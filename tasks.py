...
def build(c):
    ...
    mkdocs_config_template = Path('templates','mkdocs.yml')
    mkdocs_build_config = Path('website','mkdocs.yml')
    preamble_template = Path('templates','preamble.py')
    preamble_file = Path('website','preamble.py')
    print("Copying mkdocs.yml and preamble.py from templates/ to website/")
